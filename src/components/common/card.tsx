...
const Card: React.FC<Props> = ({
  ...
  return (
    <Link href={href} className="group flex justify-center text-center flex-col">
      ...
      <div
        className={`relative inline-flex mb-3.5 md:mb-4 lg:mb-5 xl:mb-6 mx-auto ${!disableBorderRadius && (variant === 'rounded' ? 'rounded-md' : 'rounded-xl')
          }`}
      >
        <div className="flex">
          <Image
            ...
            height={imageSize}
            quality={100}
            className={`object-cover bg-gray-300 ${!disableBorderRadius && (variant === 'rounded' ? 'rounded-md' : 'rounded-xl')}`}
          />
        </div>
        {effectActive === true && (
          <>
            <div
              className={`absolute top left bg-black w-full h-full opacity-0 transition-opacity duration-300 group-hover:opacity-30 ${!disableBorderRadius && (variant === 'rounded' ? 'rounded-md' : 'rounded-xl')
                }`}
            />
